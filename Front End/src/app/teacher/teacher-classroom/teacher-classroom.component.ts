...
export class TeacherClassroomComponent implements OnInit {
  ...
  constructor(private formBuilder : FormBuilder,
    ...
    private classService : TeacherClassroomService,
 ...
 updated = false;
  ...
  updateModal(update: any, item: CLASSROOM){
    if (item) {
      ...
      this.globalMemor.currentCodeClass = item.codeClass;
  ...
  onSubmit( body : CLASSROOM ){
    ...
    if( this.updated ){
      ...
      this.classService.updateCategory( this.id, body ).subscribe( data => {
        if (data) {
          this.toaster.success('Class Code have been updated','Success');
          this.onReset();
          this.getCategoryList();
      ...
      this.classService.createNewCategory( body ).subscribe( data => {
        if (data) {
          this.toaster.success('Create new class successfully','Success')
          this.onReset();
          this.modalService.dismissAll();
          this.getCategoryList();
          this.classRoomForm.reset();
